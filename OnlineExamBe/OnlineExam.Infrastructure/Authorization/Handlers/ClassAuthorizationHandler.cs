...
{
    public class ClassAuthorizationHandler : AuthorizationHandler<ResourceRequirement, Class>
    {
        ...
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, ResourceRequirement requirement, Class resource)
        {
            ...
            if (context.User.IsInRole("STUDENT"))
            {
                ...
                if (requirement.Action == ResourceAction.ViewDetail && resource.StudentClasses.Any(s => s.StudentId == userId))
                    context.Succeed(requirement);
            }
